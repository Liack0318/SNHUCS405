...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Regex targets patterns like OR 1=1, OR 'x'='x', OR "x"="x", etc.
  static const std::regex pneat(R"(or\s+(['\"]?[A-Za-z0-9]+['\"]?)\s*=\s*\1)",
      std::regex_constants::icase);

  if (std::regex_search(sql, pneat))
  {
      std::cerr << "SECURITY WARNING: SQL Injection attempt detected! Query blocked.\n"
          << "Query: " << sql << std::endl;
      return false;
  }

  // If no injection is detected, proceed with the query
  char* error_message;
